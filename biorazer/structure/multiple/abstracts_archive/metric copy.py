...
class Metric:
    '''
    A metric is a class that helps you to calculate and summarize a metric for a project.
    ...
    {key: "0/./fold_{sn}_model_{i}_castp"}. 0 indicates the identifier index, . indicates the folder containing structures
    {sn} is replaced with the sequence name, {i} is replaced with the sequence index.
    '''
    ...
    def parse_metric_file_path_pattern(self, metric_file_path_pattern: str, sequence_name, sequence_index):
        metric_file_path_parts = metric_file_path_pattern.split("/")
